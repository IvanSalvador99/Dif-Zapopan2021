<?php
    ...
    $query = "SELECT * FROM $persona_despensas WHERE idpersona = '" . $_GET['id'] . "'";
    //echo "Query: " . $query . "</br>";
    if ($result = mysql_query($query, $con)){
        if (mysql_num_rows($result) > 0) {
            $registrado = 1;
            $rowdespensa = mysql_fetch_array($result);
            /*var_dump($rowdespensa);
            echo "</br>";*/
            if ($rowdespensa['activo'] > 0) {
                $activo = 1;
            } else {
                $activo = 0;
            }
...
?>
                    ...
                    $.post('ajax/ajax_getservicio.php',{"servicio": $("#servicio").val()}, function(respuesta){
                        ...
                        if (respuesta.tienemodulo == 0) {
                            ...
                            $('<form>', {
                                ...
                                "method": "post",
                                "html": '<input type="hidden" id="idservicio" name="idservicio" value="' + respuesta.id + '" /><input type="hidden" id="servicio" name="servicio" value="' + respuesta.id + '" /><input type="hidden" id="idpersona" name="idpersona" value="' + <?= $_GET['id'] ?> + '" /><input type="hidden" id="locacion" name="locacion" value="' + $("#locacion").val() + '" /><input type="hidden" id="fechaservicio" name="fechaservicio" value="' + $("#fechaservicio").val() + '" />',
                                "action": 'engine/engine_registrarservicio.php'
                            }).appendTo(document.body).submit();
                        } else {
                            if (respuesta.alias == null) {
                                // alert ("Entre Aqui ifelse (1)");
                                $('<form>', {
                                    "id": "formregistrarservicios",
                                    "method": "post",
                                    "html": '<input type="hidden" id="idservicio" name="idservicio" value="' + respuesta.id + '" /><input type="hidden" id="servicio" name="servicio" value="' + respuesta.id + '" /><input type="hidden" id="idpersona" name="idpersona" value="' + <?= $_GET['id'] ?> + '" /><input type="hidden" id="locacion" name="locacion" value="' + $("#locacion").val() + '" /><input type="hidden" id="fechaservicio" name="fechaservicio" value="' + $("#fechaservicio").val() + '" />',
                                ...
                                $('<form>', {                                    
                                    "id": "formregistrarservicios",
                                    "method": "post",
                                    "html": '<input type="hidden" id="idservicio" name="idservicio" value="' + respuesta.id + '" /><input type="hidden" id="servicio" name="servicio" value="' + respuesta.alias + '" /><input type="hidden" id="idpersona" name="idpersona" value="' + <?= $_GET['id'] ?> + '" /><input type="hidden" id="locacion" name="locacion" value="' + $("#locacion").val() + '" /><input type="hidden" id="fechaservicio" name="fechaservicio" value="' + $("#fechaservicio").val() + '" />',
                                    "action": 'modulos/' + respuesta.alias + '/' + respuesta.alias + '.php'
                                }).appendTo(document.body).submit();                                
                            }
        ...
        <div class="text-center">
            ...
            <a type="button" href="engine/engine_imptarjeton.php?id=<?= $_GET['id'] ?>" target="_blank" class="btn btn-primary">Imprimir tarjeton</a>
        </div>
        <h4>Historial de servicios: <a href="#servicios" data-toggle="collapse"><span class="glyphicon glyphicon-collapse-down"></span></a></h4>
        <h4><?php echo (($registrado == 1) ? "♦ Esta persona estuvo registrada en el programa de despensas municipales 2018" : ""); ?></h4>
        <div>
                ...
                if ($result = mysql_query($query, $con)) {
                    if (mysql_num_rows($result) > 0) {                        
                        ...
                        while ($rowservicio = mysql_fetch_array($result)) { 
                            if ($rowservicio['idorginalservicio'] == 478) {                                
                                $output .= "<tr id='" . $rowservicio['id'] . "'><td>" . $rowservicio['id'] . "</td><td>" . $rowservicio['fecha'] . "</td><td>" . $rowservicio['locacion'] . "</td><td> Atención psicológica UAVV </td><td>" . $rowservicio['descripcion'] . "</td><td>coso</td></tr>";
                            }
                            if ($rowservicio['idorginalservicio'] == 479) {                                
                                $output .= "<tr id='" . $rowservicio['id'] . "'><td>" . $rowservicio['id'] . "</td><td>" . $rowservicio['fecha'] . "</td><td>" . $rowservicio['locacion'] . "</td><td> 	Asitencia social UAVV </td><td>" . $rowservicio['descripcion'] . "</td><td>coso</td></tr>";
                            } 
                            else {
