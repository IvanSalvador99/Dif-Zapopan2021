                ...
                if (mysql_num_rows($result) > 0) {
                    ...
                    if ($rowincidencia['estatus'] == 1) {
                            ...
                            if (($permitido && $rowusuario['id'] != $_SESSION['padron_admin_id']) || $_SESSION['padron_admin_permisos'] == 1) {
                                if ($rowincidencia['concepto'] == 4) {
                                    ...
                                    if ($restante >= 0) {
                                        ...
                                        if (mysql_query($query, $con)) {
                                            $query = "UPDATE $incidencias SET historial='" . mysql_real_escape_string($rowincidencia['historial']) . "\n\n" . date("Y-m-d H:i:s") . " -- Incidencia aprobada por Jefe de departamento o director. Se deduce un día economico, habia: " . $rowusuario['diaseconomicos'] . ", restantes: $restante', estatus='2', idaprobado='" . $_SESSION['padron_admin_id'] . "' WHERE id ='" . $rowincidencia['id'] . "'";
                                            if (mysql_query($query, $con)) {
                                                if (mysql_affected_rows() > 0) {
                                                    echo "Success|Incidencia actualizada correctamente.";
                                                    exit();
                                        ...
                                        echo "Fail|El usuario no cuenta con dias economicos";
                                        exit();
                                    }
                                } else {
                                    $query = "UPDATE $incidencias SET historial='" . mysql_real_escape_string($rowincidencia['historial']) . "\n\n" . date("Y-m-d H:i:s") . " -- Incidencia aprobada por Jefe de departamento o director.', estatus='2', idaprobado='" . $_SESSION['padron_admin_id'] . "' WHERE id ='" . $rowincidencia['id'] . "'";
                        ...
                        if (esAdminIncidencias($_SESSION['padron_admin_id']) || $_SESSION['padron_admin_permisos'] == 1) {
                            $query = "UPDATE $incidencias SET historial='" . mysql_real_escape_string($rowincidencia['historial']) . "\n\n" . date("Y-m-d H:i:s") . " -- Incidencia aprobada por oficina de nominas.', estatus='3', idnominas = '" . $_SESSION['padron_admin_id'] . "' WHERE id ='" . $rowincidencia['id'] . "'";
                            if (mysql_query($query, $con)) {
                                if (mysql_affected_rows() > 0) {
