...
if (isset($_GET['action']) && $_GET['action'] == 1) {
    ...
    alerta_bota("Incidencia actualizada correctamente2", 0);
} elseif (isset($_GET['action']) && $_GET['action'] == 3) {
    alerta_bota("Los usuarios de confianza no cuentan con dias economicos y horas bimestrales", 0);
} elseif (isset($_GET['action']) && $_GET['action'] == 4) {
    alerta_bota("Esta incidencia no puede ser editada pues ya esta finalizada.", 0);
} elseif (isset($_GET['action']) && $_GET['action'] == 5) {
    alerta_bota("Esta incidencia no puede ser capturada por ser extemporánea.", 0);
}
?>
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#rechazarbtn").on("click", function() {
            $.post('ajax/ajax_rechazarincidencia.php', {
                "idincidencia": $("#idmodalr").text(),
                "razon": $("#descripcionrechazo").val()
            }, function(respuesta) {
        ...
        $("#cancelarbtn").on("click", function() {
            $.post('ajax/ajax_cancelarincidencia.php', {
                "idincidencia": $("#idmodalc").text(),
                "razon": $("#descripcioncancela").val()
            }, function(respuesta) {
                var result = respuesta.split("|");
                if (result[0] == "Success") {
                    alert(result[1]);
        ...
        $("#btnjefesecre").on("click", function() {
            //alert ("Hola");
            window.location.href = "jefesecretaria.php";
        });

        $(".table tr").on("dblclick", function(e) {
            if ($(this).attr("id") !== undefined) {
                $.post('ajax/ajax_getincidencia.php', {
    ...
    function aceptarclick(id, event) {
        ...
        if (sure) {
            $.post('ajax/ajax_aceptarincidencia.php', {
                "idincidencia": id
            }, function(respuesta) {
                var result = respuesta.split("|");
                if (result[0] == "Success") {
...
?>
    <br />
    <div class="row text-center">
        <a href="incidenciaregistro.php" class="btn btn-primary" id="btnnueva">Nueva incidencia</a>
    </div>
    <br />
<?php
}
...
<div class="row">
    ...
    if (esAdminIncidencias($_SESSION['padron_admin_id']) || $_SESSION['padron_admin_permisos'] == 1) {
        echo '<div class="col-md-6 text-center"><a class="btn btn-primary" id="btnexportar" style="margin-left: 5px; margin-right: 5px;">Exportar <span class="glyphicon glyphicon-export"></span></a><a class="btn btn-primary" id="btnjefesecre" style="margin-left: 5px; margin-right: 5px;">Asignar jefes y/o secretarias <span class="glyphicon glyphicon-copy"></span></a><a href="#!" class="btn btn-primary" id="btncapturaext" style="margin-top: 5px">Activar captura extemporanea <span class="glyphicon glyphicon-calendar"></span></a></div>';
    }
    ?>
</div>
<br />
    ...
    <table class="table table-striped table-condensed">
        ...
        <tbody class="resultBusqueda">
            ...
            if ($_SESSION['padron_admin_permisos'] == 1 || esAdminIncidencias($_SESSION['padron_admin_id'])) {
                $query = "SELECT * FROM $vw_incidencias WHERE fechacaptura >= '" . date("Y-m-d", strtotime("-3 months")) . "' LIMIT 0, 50";
