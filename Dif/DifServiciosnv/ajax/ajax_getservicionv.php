<?php
    ...
    $query = "SELECT * FROM $serviciosnv_servicios where iddepartamento = '$departamento'";
            ...
            if ($catalogo[$i][2] == 0) {
                for ($j = 0; $j < count($catalogo); $j++) { 
                    if ($catalogo[$j][2] == $catalogo[$i][0]) {
                        if (!$flag) {
                            if ($catalogo[$j][0] == '11' || $catalogo[$j][0] == '55') {
                                $output .= "<optgroup label = '" . $catalogo[$i][1] . "'>";
                                $output .= "<option value = '" . $catalogo[$j][0] . "' disabled>" . $catalogo[$j][1] . "</option>";
                                $flag = true;
                            } else {
                                $output .= "<optgroup label = '" . $catalogo[$i][1] . "'>";
