<?php
            ...
            if ($result = mysql_query($query, $con)) {
                ...
                if ($result = mysql_query($query, $con)) {
                    if (mysql_num_rows($result) > 0 ){
                        $rowsicats = mysql_fetch_array($result);
                        /*echo "</br></br>";
                        var_dump($rowsicats);*/
                        $haysicats = true;
                    }
    ...
    $pdf->Image('../../../../imagenes/LOGOTIPO_DIF Zapopan.png', 10, 4, 20);
    ...
    $pdf->Cell(0, $multiCellHeight, iconv('UTF-8', 'windows-1252', $rowservicio['servicio']), 1, 1, "L", false);
    $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', 'Apoyo(s) y/o servicio(s) otorgado(s)'), 1, 1, "C", true);
    
    $query = "SELECT GROUP_CONCAT(persona_cat_apoyo.apoyosservicios SEPARATOR ',') AS apoyos
                FROM persona_trabajo_social
                INNER JOIN persona_cat_apoyo ON FIND_IN_SET(persona_cat_apoyo.id, persona_trabajo_social.apoyo) > 0
                WHERE persona_trabajo_social.idservicio = '" . $_GET['idservicio'] . "'";
    ...
    $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['numero_canalizacion']), 1, 1, "C", false);
    $pdf->Ln(1);
    $pdf->SetY(260);
    $pdf->Cell(95, 4, iconv('UTF-8', 'windows-1252', 'TS. MARIA DEL CARMEN RUBI CAMACHO'), 0, 0, "C", false);
    ...
    $pdf->Cell(95, 4, iconv('UTF-8', 'windows-1252', 'BENEFICIARIO'), 0, 1, "C", false);
    ...
    if ($haysicats) {
        ...
        $oldY = $pdf->GetY();
        if ($rowsicats['origen'] == NULL) {
            $pdf->MultiCell(80, 4, iconv('UTF-8', 'windows-1252', 'DIF ZAPOPAN'), 0, "C", false);
        } else {
            $pdf->MultiCell(80, 4, iconv('UTF-8', 'windows-1252', $rowsicats['origen']), 0, "C", false);
        }
        $multiCellHeight = $pdf->GetY() - $oldY;
        $pdf->SetXY(110, $oldY);
        $pdf->Cell(80, $multiCellHeight, iconv('UTF-8', 'windows-1252', $rowsicats['encargado_origen']), 0, 1, "C");
        ...
        $oldY = $pdf->GetY();
        $pdf->MultiCell(80, 4, iconv('UTF-8', 'windows-1252', $rowsicats['destino']), 0, "C", false);
        $multiCellHeight = $pdf->GetY() - $oldY;
        $pdf->SetXY(110, $oldY);
        $pdf->Cell(80, $multiCellHeight, iconv('UTF-8', 'windows-1252', $rowsicats['encargado_destino']), 0, 1, "C");
        $pdf->Line(20, $pdf->GetY(), 100, $pdf->GetY());
        $pdf->Line(110, $pdf->GetY(), 190, $pdf->GetY());
        $pdf->SetXY(20, $pdf->GetY() -0.5);
        $pdf->Cell(80, 4, iconv('UTF-8', 'windows-1252', 'Nombre de la institución'), 0, 0, "C");
        ...
        $pdf->Cell(40, 4, iconv('UTF-8', 'windows-1252', $rowsicats['numero_canalizacion']), 1, 0, "L", false);
        $pdf->Cell(30, 4, iconv('UTF-8', 'windows-1252', 'N° de registro'), 1, 0, "L", true);
        $pdf->Cell(40, 4, iconv('UTF-8', 'windows-1252', $rowsicats['numero_registro']), 1, 0, "L", false);
        $pdf->Cell(20, 4, iconv('UTF-8', 'windows-1252', 'Fecha'), 1, 0, "L", true);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['fecha']), 1, 1, "L", false);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', 'DATOS GENERALES'), 1, 1, "L", true);
        $oldY = $pdf->GetY();
        ...
        $pdf->MultiCell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['solicitud']), 1, "J", false);
        ...
        $pdf->MultiCell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['evolucion_caso']), 1, "J", false);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', 'SE ANEXA LA SIGUIENTE DOCUMENTACIÓN'), 1, 1, "C", true);
        $pdf->MultiCell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['anexos']), 1, "J", false);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', 'OBSERVACIONES Y/O SUGERENCIAS'), 1, 1, "C", true);
        $pdf->MultiCell(0, 4, iconv('UTF-8', 'windows-1252', $rowsicats['observaciones_sugerencias']), 1, "J", false);
        $pdf->Cell(40, 4, iconv('UTF-8', 'windows-1252', 'Responsable del caso:'), 1, 0, "L", true);
        $pdf->Cell(70, 4, iconv('UTF-8', 'windows-1252', "TS. " . $rowsicats['responsable']), 1, 0, "L", false);
        $pdf->Cell(40, 4, iconv('UTF-8', 'windows-1252', 'Al programa de:'), 1, 0, "L", true);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', $rowservicio['servicio']), 1, 1, "L", false);
        $pdf->Cell(20, 4, iconv('UTF-8', 'windows-1252', 'Telefono:'), 1, 0, "L", true);
        ...
        $pdf->Cell(20, 4, iconv('UTF-8', 'windows-1252', $rowsicats['extension']), 1, 0, "L", false);
        ...
        $pdf->Ln(10);
        $pdf->Cell(90, 4, iconv('UTF-8', 'windows-1252', 'Mtra. Dora Aida Vargas Ocegueda'), 0, 0, "C", false);
        $pdf->SetX(120);
        $pdf->Cell(0, 4, iconv('UTF-8', 'windows-1252', $rowpersona['apaterno'] . " " . $rowpersona['apaterno'] . " " . $rowpersona['nombre']), 0, 1, "C", false);
        $oldY = $pdf->GetY();
